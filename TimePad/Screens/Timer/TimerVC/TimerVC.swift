...
final class TimerVC: UIViewController {
  ...
  override func viewDidDisappear(_ animated: Bool) {
    ...
    updateWorkModel(
      id: workModel.id!, hour: timerView.lastHour!, minute: timerView.lastMinute!,
      seconds: timerView.lastSeconds!)
    createOrUpdateLastWorkModel()
  }

  //MARK: Coredata
  private func updateWorkModel(id: String, hour: Int, minute: Int, seconds: Int) {
    coreDataManager.updateWork(
      by: id, newTitle: workModel.title!, newHour: hour, newMinute: minute, newSeconds: seconds,
      newType: workModel.type!)
  }

  private func createOrUpdateLastWorkModel() {
    coreDataManager.createOrUpdateLastWork(
      title: workModel.title!, hour: timerView.lastHour!, minute: timerView.lastMinute!,
      seconds: timerView.lastSeconds!, type: workModel.type!)
  }
  ...
  private func addTimer() {
    ...
    NSLayoutConstraint.activate([
      ...
      timerView.centerYAnchor.constraint(
        equalTo: view.centerYAnchor, constant: -view.frame.height / 4),
    ])
