...
extension GraphDetailVM: GraphDetailVMProtocol {
    ...
    func filterData(for scope: TimeScope) -> [DataPoint] {
        ...
        switch scope {
        case .daily:
            return dataPoints.filter { calendar.isDate($0.date, inSameDayAs: now) }
        case .monthly:
            return dataPoints.filter { calendar.isDate($0.date, equalTo: now, toGranularity: .month) }
        case .yearly:
            return dataPoints.filter { calendar.isDate($0.date, equalTo: now, toGranularity: .year) }
        }
    }
