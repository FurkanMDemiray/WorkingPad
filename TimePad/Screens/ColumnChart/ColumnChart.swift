...
private enum Colors {
  static let work = UIColor.hexStringToUIColor(hex: Colors.red)
  static let workout = UIColor.hexStringToUIColor(hex: Colors.blue)
  static let reading = UIColor.hexStringToUIColor(hex: Colors.orange)
  static let coding = UIColor.hexStringToUIColor(hex: Colors.green)
}
...
class ColumnChart: UIView {
    ...
    private let colors: [UIColor] = [Colors.work, Colors.workout, Colors.reading, Colors.coding]
    ...
    override func layoutSubviews() {
        ...
        for i in 0..<columns.count {
            ...
            columns[i].frame = CGRect(x: x,
                                    y: maxHeight - columnHeight,
                                    width: columnWidth,
                                    height: columnHeight)
            
            labels[i].frame = CGRect(x: x,
                                   y: maxHeight + 5,
                                   width: columnWidth,
                                   height: 30)
        }
    ...
    @objc private func columnTapped(_ sender: UITapGestureRecognizer) {
        guard let column = sender.view,
              let index = columns.firstIndex(of: column) else { return }
